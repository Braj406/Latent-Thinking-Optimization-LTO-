...
# 2. Import from your modular repository (Using Qwen Classifier!)
from training.classifiers import QwenLatentClassifier
...
humaneval_file = '/content/drive/MyDrive/chumaneval_lto_dataset_qwenpt'
mbpp_file = '/content/drive/MyDrive/cqwenmbpp_lto_dataset_RESCUED.pt'
...
# 6. Initialize & Train Qwen LRM
device = 'cuda' if torch.cuda.is_available() else 'cpu'
detected_num_layers = train_tensors.shape[1]
detected_hidden_dim = train_tensors.shape[2]

model = QwenLatentClassifier(hidden_dim=detected_hidden_dim, num_layers=detected_num_layers, dropout=0.5)
